...
def build_salary():
    ...
    allFiles = glob.glob(path + "/dk_*.csv")
    ...
    for idx,item in df.iterrows():
            ...
            if player is not None:
                ...
                try:
                   ...
                   _line["salary"] = item["salary"].replace(",","")
                   _line["fantasy_pts"] = item["fantasy_pts"]
                   _line["Player_ID"] = player.PERSON_ID
                   session.add(Salary(**_line))
                   session.commit()
                except Exception as e:
                   print("Failed to create salary: %s" %(_line))
                   failed.append(_line)
                   session.rollback()
